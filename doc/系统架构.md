# NRL 系统架构文档

> NRL (Network Radio Link) —— 通过网络连接无线电，实现跨地域、跨设备的语音互联系统

---

## 一、系统总览

```mermaid
graph TB
    subgraph APRS_NET["APRS 网络（全球业余无线电分组网络）"]
        APRSIS["APRS-IS 服务器\nrotate.aprs2.net:14580"]
        APRSTV["aprs.tv / aprs.fi\n全球节点查询与地图展示"]
        APRSIS <--> APRSTV
    end

    subgraph INTERNET["互联网"]
        direction TB

        subgraph SRV_A["NRL 服务器 A\nLinux / CentOS"]
            R999A["房间 999\n全网互通"]
            R1000A["房间 1000+\n公共房间"]
            R1_3A["房间 1-3\n用户私有"]
        end

        subgraph SRV_B["NRL 服务器 B\nDocker 容器"]
            R999B["房间 999\n全网互通"]
            R1000B["房间 1000+\n公共房间"]
            R1_3B["房间 1-3\n用户私有"]
        end

        subgraph SRV_C["NRL 服务器 C\nWindows"]
            R999C["房间 999\n全网互通"]
            R1000C["房间 1000+\n公共房间"]
            R1_3C["房间 1-3\n用户私有"]
        end

        R999A <-->|"语音互转发\n(经 APRS 自动发现)"| R999B
        R999B <-->|"语音互转发"| R999C
        R999A <-->|"语音互转发"| R999C
    end

    subgraph CLIENTS_HW["硬件终端"]
        BOX["马工盒子\nNRL2100/2200/2300/3188/2600\n射频 ↔ 网络\nAPRS 标识: NRLBOX"]
        ESP32["ESP32 终端\n爱好者自制\n全双工 / 带屏或无屏\nAPRS 标识: NRLBOX"]
    end

    subgraph CLIENTS_RF["无线电设备（本地 RF）"]
        RADIO["模拟中继台 / 公网台"]
        HT["手台 / 对讲机\n（单工）"]
    end

    subgraph CLIENTS_APP["软件终端"]
        APP_ANDROID["73HAM\nAndroid App\nAPRS 标识: NRLAPP"]
        APP_IOS1["NRL互联\niOS App\nAPRS 标识: NRLAPP"]
        APP_IOS2["NRL工具集\niOS App\nAPRS 标识: NRLAPP"]
    end

    subgraph AUX["辅助工具"]
        NANNY["NRLNANNY 保姆软件\n录音 / 定时信标 / 实时监听"]
        ENANNY["增强版保姆\n爱好者开发\n多服务器多房间并发监听"]
    end

    HT -->|"RF 音频"| BOX
    RADIO -->|"RF 音频"| BOX

    BOX -->|"NRL 协议\nUDP 60050"| SRV_A
    ESP32 -->|"NRL 协议\nUDP 60050"| SRV_A
    APP_ANDROID -->|"NRL 协议"| SRV_B
    APP_IOS1 -->|"NRL 协议"| SRV_B
    APP_IOS2 -->|"NRL 协议"| SRV_C

    NANNY -->|"NRL 协议"| SRV_A
    ENANNY -->|"NRL 协议"| SRV_A
    ENANNY -->|"NRL 协议"| SRV_B
    ENANNY -->|"NRL 协议"| SRV_C

    SRV_A -->|"APRS 位置+状态上报\nNRLSRV"| APRSIS
    SRV_B -->|"APRS 位置+状态上报\nNRLSRV"| APRSIS
    SRV_C -->|"APRS 位置+状态上报\nNRLSRV"| APRSIS
    BOX -->|"APRS 位置上报\nNRLBOX"| APRSIS
    APP_ANDROID -->|"APRS 位置上报\nNRLAPP"| APRSIS

    SRV_A -->|"查询活跃服务器列表\n自动发现互联节点"| APRSTV
    SRV_B -->|"查询活跃服务器列表"| APRSTV
```

---

## 二、服务器房间（群组）结构

```mermaid
graph LR
    subgraph SERVER["NRL 服务器"]
        direction TB

        subgraph PRIV["私有房间（每用户独占）"]
            R1["房间 1\n用户私有"]
            R2["房间 2\n用户私有"]
            R3["房间 3\n用户私有"]
        end

        subgraph GLOBAL["全网互通房间"]
            R999["房间 999\n🌐 全网互通\n语音自动转发至所有服务器"]
        end

        subgraph PUBLIC["公共房间（1000+）"]
            R1000["房间 1000\n公共频道"]
            R1001["房间 1001\n公共频道"]
            RDOT["..."]
            RN["房间 N\n公共频道"]
        end
    end

    note1["• 先到先得发言权\n• PTT 松开 200ms 后释放"]
    note2["• 任意用户可加入\n• 不同设备/用户可在此通联"]
    note3["• 仅房间归属用户可使用\n• 其他人无法访问"]

    PRIV --- note3
    GLOBAL --- note1
    PUBLIC --- note2
```

### 房间类型说明

| 房间范围 | 类型 | 访问权限 | 说明 |
| --- | --- | --- | --- |
| 1 – 3 | 用户私有 | 仅该用户 | 每个用户独享，外人无法进入 |
| 999 | 全网互通 | 所有服务器用户 | 发言后自动转发至所有互联服务器的 999 房间 |
| 1000+ | 公共房间 | 所有用户 | 多用户多设备在同一房间内通联 |
| 任意 | 会议模式 | 同房间用户 | 开启后切换为全双工 PCM 混音模式 |

### 发言机制详述

语音发言模式由**在线设备数**和**房间类型**共同决定，服务器自动切换，无需手动配置。

#### 按在线设备数自动切换

| 在线设备数 | 模式 | 行为说明 |
| --- | --- | --- |
| 1 台（非中继类型） | 🔔 回音反射 | 服务器将该设备的音频原路反射回去，设备可听到自身语音，便于自检 |
| 2 台 | 📞 自动全双工 | 服务器自动切换为全双工模式；ESP32 / App 类终端无需按住 PTT，可像电话一样自由通联 |
| 3 台及以上 | 📻 默认单工 | 先到先得抢占发言权，PTT 松开后 **200ms** 释放，其他设备方可发言 |

> 所有模式下语音释放时间均为 **200ms**（松开 PTT 后）。

#### 会议模式（Conference Mode）

任意房间均可由管理员开启会议模式，开启后覆盖上述单工/全双工规则：

- **服务器端 PCM 混音**：对同时到达的多路音频进行实时叠加混音
- **全双工广播**：混音结果转发给房间内所有在线设备
- **无发言抢占**：所有人可同时讲话，互不阻塞

---

## 三、设备与客户端

```mermaid
graph TB
    subgraph HW["硬件终端"]
        subgraph MAGONG["马工盒子（NRL 系列）"]
            direction LR
            M_RF["射频接口\nRF In/Out"]
            M_NET["网络接口\nNRL 协议"]
            M_CODEC["音频编解码\nG.711"]
            M_RF -->|"单工 PTT"| M_CODEC
            M_CODEC -->|"UDP 60050"| M_NET
        end

        subgraph ESP32_DEV["ESP32 终端（爱好者自制）"]
            direction LR
            E_MIC["麦克风 / 喇叭\n全双工"]
            E_DISPLAY["显示屏（可选）\n房间/状态显示"]
            E_NET2["NRL 协议\nWiFi 连接"]
            E_MIC <-->|"全双工"| E_NET2
        end
    end

    subgraph SW["软件终端"]
        subgraph ANDROID["Android"]
            APP73["73HAM App\n全功能对讲"]
        end
        subgraph IOS["iOS"]
            NRLIOS["NRL互联 App"]
            NRLTOOL["NRL工具集 App"]
        end
    end

    subgraph AUX2["辅助软件"]
        subgraph NANNY2["NRLNANNY 保姆"]
            N1["房间录音"]
            N2["定时播放语音信标"]
            N3["实时播放房间语音"]
        end

        subgraph ENANNY2["增强版保姆（爱好者开发）"]
            EN1["同时连接多台服务器"]
            EN2["同时监听多个房间"]
            EN3["多路中继台并发监听"]
            EN4["语音流聚合"]
        end
    end

    subgraph RF_LOCAL["本地无线电（本地连接到马工盒子）"]
        RELAY["模拟中继台\nVHF/UHF"]
        WALKIE["手台/对讲机\n单工 PTT"]
    end

    WALKIE -->|"RF 射频"| MAGONG
    RELAY -->|"RF 射频"| MAGONG
```

---

## 四、全网互通（999 房间）数据流

```mermaid
sequenceDiagram
    participant U as 用户设备
    participant SA as 服务器 A (999)
    participant SB as 服务器 B (999)
    participant SC as 服务器 C (999)
    participant OA as 服务器A其他设备
    participant OB as 服务器B其他设备
    participant OC as 服务器C其他设备

    U->>SA: PTT 按下，发送语音帧 (UDP)
    SA->>OA: 转发给本服务器 999 房间内其他设备
    SA->>SB: 跨服务器转发语音帧
    SA->>SC: 跨服务器转发语音帧
    SB->>OB: 转发给服务器B 999 房间内设备
    SC->>OC: 转发给服务器C 999 房间内设备
    U->>SA: PTT 释放
    Note over SA,SC: 语音结束 1 秒后\n抢发言权重置
```

---

## 五、马工盒子射频桥接数据流

```mermaid
flowchart LR
    subgraph RF_SIDE["无线电侧（单工）"]
        HT2["手台/对讲机"]
        REPEATER["中继台"]
    end

    subgraph BOX["马工盒子"]
        PTT_DET["PTT 检测\n载波检测 CTCSS/DCS"]
        CODEC["G.711 编解码"]
        NRL_CLIENT["NRL 客户端"]
    end

    subgraph NET_SIDE["网络侧"]
        SERVER2["NRL 服务器\n房间（群组）"]
    end

    HT2 -->|"RF 语音 + PTT"| PTT_DET
    REPEATER -->|"RF 语音"| PTT_DET
    PTT_DET --> CODEC
    CODEC -->|"G.711 编码帧"| NRL_CLIENT
    NRL_CLIENT -->|"UDP 60050"| SERVER2

    SERVER2 -->|"UDP 60050\n语音帧"| NRL_CLIENT
    NRL_CLIENT --> CODEC
    CODEC -->|"解码音频"| REPEATER

    Note1["⚠️ 注意：对讲机为单工，\n不支持全双工操作"]
```

---

## 六、部署架构

```mermaid
graph TB
    subgraph DEPLOY["部署方式"]
        subgraph COMPOSE["Docker Compose（推荐，nrllink-web 仓库）"]
            C1["前端容器 web\nnginx，对公网监听 80"]
            C2["后端容器 nrllink\n接口 9000（局域网）"]
            C3["同一网络组 nrllink-net\n服务名互访 nrllink:9000"]
            C4["数据卷\nnrllink-data / nrllink-uploads"]
            C1 -->|"/api /ws /uploads"| C3 --> C2
            C2 --> C4
        end

        subgraph SINGLE["单独跑后端（本仓库 compose）"]
            S1["docker pull ghcr.io/78ham/nrllink:latest"]
            S2["接口 9000（局域网）\n设备 UDP 60050（公网）"]
            S3["自定义配置（可选）\n/nrllink/conf/udphub.yaml"]
            S1 --> S2 --> S3
        end

        subgraph NATIVE["Linux 原生 / Windows"]
            N1["systemctl nrllink.service\n或直接运行可执行文件"]
            N2["接口 9000\n设备 UDP 60050"]
            N1 --> N2
        end
    end

    subgraph PORTS["端口说明"]
        P1["TCP 80 — 前端容器（公网，页面入口）"]
        P2["TCP 9000 — 后端接口 / WebSocket（本机 / 局域网，不要暴露公网）"]
        P3["UDP 60050 — 设备语音 / 控制信令（公网直通，不走 HTTP 反代）"]
    end

    CLIENT["公网浏览器"] -->|"TCP 80"| C1
    LAN["局域网客户端"] -->|"TCP 9000"| C2
    DEVICE["NRL 设备 / App"] -->|"UDP 60050"| C2
    NOTE["可选：宿主机自备反向代理\n提供域名 + TLS，指向前端容器 80"]
    CLIENT -.-> NOTE

    COMPOSE --> PORTS
    SINGLE --> PORTS
    NATIVE --> PORTS
```

---

## 七、系统组件清单

### 服务器端

| 组件 | 说明 |
| ---- | ---- |
| NRL 服务器 | 核心中转服务，管理房间/设备/用户，转发语音帧 |
| Web 管理后台 | 用户注册、设备管理、房间配置（内部 TCP 80，经 nginx HTTPS 443 对外发布） |
| 信令/语音通道 | UDP 60050，低延迟语音转发 |
| SQLite 数据库 | 用户、设备、群组、日志持久化存储 |
| 服务器互联模块 | 999 房间跨服务器语音转发 |

### 硬件终端

| 设备 | 型号 | 特性 |
| ---- | ---- | ---- |
| 马工盒子 | NRL2100/2200/2300/3188/2600 | 射频桥接，单工，G.711 编解码 |
| ESP32 终端 | 爱好者自制 | 全双工，可选显示屏，WiFi 连接 |

### 软件终端

| 平台 | 应用 | 说明 |
| ---- | ---- | ---- |
| Android | 73HAM | 全功能 NRL 对讲客户端 |
| iOS | NRL互联 | iOS NRL 客户端 |
| iOS | NRL工具集 | iOS NRL 工具客户端 |

### 辅助软件

| 软件 | 功能 |
| ---- | ---- |
| NRLNANNY 保姆 | 房间录音、定时语音信标播放、实时语音监听 |
| 增强版保姆 | 同时连接多服务器多房间、多路中继台并发监听、语音聚合 |

---

## 八、典型应用场景

### 场景一：本地中继台接入全网

```text
本地中继台 → 马工盒子 → NRL服务器A(999房间) → 所有互联服务器(999) → 全网用户
```

### 场景二：手机用户与无线电用户通联

```text
手机 73HAM App → 服务器B(1000号房间) ← ESP32终端 ← 本地对讲机(通过马工盒子)
```

### 场景三：爱好者监听多地中继台

```text
增强版保姆 → 同时连接 服务器A(999) + 服务器B(1000) + 服务器C(1001)
           → 各服务器对应连接了不同地区的中继台
           → 实现多地中继台同步监听
```

### 场景四：用户私有通信

```text
用户设备A(房间1) → 服务器(房间1，私有) → 用户设备B(同一用户，房间1)
外部用户无法进入 1-3 号房间
```

---

## 九、APRS 集成与自动发现

### 9.1 APRS 是什么

APRS（Automatic Packet Reporting System，自动分组报告系统）是业余无线电领域广泛使用的实时数字通信网络，可传播位置、状态、消息等信息，并通过互联网网关（APRS-IS）实现全球汇聚。NRL 系统利用 APRS-IS 实现节点的自动发布与发现，无需手动配置服务器列表。

### 9.2 各类节点的 APRS 标识（Destination / tocall）

| APRS 目的地标识 | 对应节点类型 |
| -------------- | ----------- |
| `NRLSRV` | NRL 服务器 |
| `NRLBOX` | 马工盒子及爱好者自制硬件终端（ESP32 等） |
| `NRLAPP` | Android / iOS 手机 App（73HAM、NRL互联等） |
| `NRLMP` | 微信小程序客户端 |

### 9.3 服务器 APRS 上报机制

```mermaid
sequenceDiagram
    participant SRV as NRL 服务器
    participant APRSIS as APRS-IS 服务器\n(rotate.aprs2.net:14580)
    participant APRSTV as aprs.tv API

    SRV->>APRSIS: TCP 连接 + 呼号认证\nuser BH4TDV-9 pass 12345 vers NRL 1.0
    Note over SRV,APRSIS: 认证成功后开始上报

    SRV->>APRSIS: 位置包（立即）\nBH4TDV-9>NRLSRV,TCPIP*:!2330.00N/11300.00EI @udp://182.92.x.x:60050,NRL互联服务器
    SRV->>APRSIS: 状态包（立即）\nBH4TDV-9>NRLSRV,TCPIP*:>在线:5,高峰:20,主站名称

    Note over SRV: 等待 1 分钟
    SRV->>APRSIS: 位置包 + 状态包（再次上报）

    Note over SRV: 等待 5 分钟
    SRV->>APRSIS: 位置包 + 状态包

    loop 每 10 分钟
        SRV->>APRSIS: 位置包 + 状态包（保活）
    end

    Note over SRV: 另一个定时任务（每 60 秒）
    SRV->>APRSTV: POST /api/findnrl?dest=NRLSRV&duration=60
    APRSTV-->>SRV: 返回全网活跃服务器列表\n[{host, port, name, online, total}]
    SRV->>APRSTV: POST /api/findnrltotal?duration=60
    APRSTV-->>SRV: 返回全网统计\n{NRLSRV:N, NRLBOX:N, NRLAPP:N, NRLMP:N}
```

### 9.4 APRS 数据包格式说明

**位置包**（让全球爱好者在 aprs.fi / aprs.tv 地图上看到服务器位置）：

```text
BH4TDV-9>NRLSRV,TCPIP*:!2330.00N/11300.00EI @udp://182.92.158.141:60050,NRL互联服务器
```

| 字段 | 说明 |
| ---- | ---- |
| `BH4TDV-9` | 呼号-SSID（运营者的业余无线电呼号） |
| `NRLSRV` | APRS 目的地标识，用于过滤查询 NRL 服务器 |
| `TCPIP*` | 经由 APRS-IS 互联网网关发送 |
| `!...I` | 位置格式（纬度/经度，I 为符号代码） |
| `@udp://...` | 服务器接入地址，供其他节点自动解析连接 |

**状态包**（上报实时在线数据）：

```text
BH4TDV-9>NRLSRV,TCPIP*:>在线:5,高峰:20,我的NRL平台
```

| 字段 | 说明 |
| ---- | ---- |
| `在线:5` | 当前在线设备数 |
| `高峰:20` | 历史最高同时在线设备数 |
| `我的NRL平台` | 平台自定义名称 |

### 9.5 双机制服务器自动发现流程

NRL 服务器通过两套机制并行发现其他服务器，互为补充：

```mermaid
flowchart TD
    START["NRL 服务器启动"] --> BOTH["同时启动两套发现机制"]

    BOTH --> APRS_BRANCH["APRS 机制（主要）"]
    BOTH --> WEB_BRANCH["nrlptt.com 官网机制（备用）"]

    subgraph APRS_FLOW["APRS 发现流程"]
        APRS_BRANCH --> AC{"APRS 配置完整?"}
        AC -- 否 --> ASKIP["跳过 APRS"]
        AC -- 是 --> AE["连接 APRS-IS 服务器\nTCP 认证"]
        AE --> AF["定时上报位置包+状态包\n目的地: NRLSRV"]
        AF --> AG["aprs.tv 收录，全球地图可见"]
        AH["每 60 秒查询 aprs.tv\n/api/findnrl?dest=NRLSRV"] --> AI["获取活跃服务器列表"]
        AE --> AH
    end

    subgraph WEB_FLOW["官网发现流程"]
        WEB_BRANCH --> WC{"SelfAddress/Port 已配置?"}
        WC -- 否 --> WSKIP["跳过官网同步"]
        WC -- 是 --> WD["启动时立即上报 + 同步"]
        WD --> WE["POST nrlptt.com\n/api/platform-servers/report"]
        WD --> WF["GET nrlptt.com\n/api/platform-servers"]
        WE2["每 3 分钟"] --> WE
        WF2["每 5 分钟"] --> WF
        WD --> WE2
        WD --> WF2
    end

    AI --> MERGE["合并更新 PlatformList"]
    WF --> MERGE
    MERGE --> FWD["999 房间语音自动转发至所有互联服务器"]
```

| 对比项 | APRS 机制 | nrlptt.com 官网机制 |
| ------ | --------- | ------------------- |
| 定位 | 主要机制 | 备用机制 |
| 上报周期 | 立即 → 1分钟 → 5分钟 → 每10分钟 | 启动立即，之后每 3 分钟 |
| 同步周期 | 每 60 秒（查询 aprs.tv） | 启动立即，之后每 5 分钟 |
| 中转服务 | APRS-IS + aprs.tv | nrlptt.com 官网直连 |
| 附加功能 | 含 GPS 位置，全球地图可见 | 仅服务器列表 |
| 需要配置 | 呼号、位置、APRS 服务器 | 仅 SelfAddress + SelfPort |

### 9.6 APRS 配置参数

| 配置项 | 说明 | 示例 |
| ------ | ---- | ---- |
| `APRSServerHost` | APRS-IS 服务器地址 | `rotate.aprs2.net` |
| `APRSServerPort` | APRS-IS 服务器端口 | `14580` |
| `CallSign` | 运营者业余无线电呼号 | `BH4TDV` |
| `SSID` | 副台标识（区分同一呼号的不同设备） | `9` |
| `Passcode` | APRS 认证码（留 0 则自动由呼号计算） | `0` |
| `SelfAddress` | 本服务器公网 IP / 域名（用于自身过滤） | `182.92.158.141` |
| `SelfPort` | 本服务器 UDP 端口 | `60050` |
| `Latitude` | 服务器所在地纬度（十进制度） | `23.5` |
| `Longitude` | 服务器所在地经度（十进制度） | `113.0` |
| `Altitude` | 高度（可选） | `50` |

### 9.7 爱好者如何通过 APRS 找到 NRL 节点

1. 访问 [aprs.tv](https://aprs.tv) 或 [aprs.fi](https://aprs.fi)
2. 搜索 APRS 目的地 `NRLSRV`（服务器）、`NRLBOX`（盒子）、`NRLAPP`（App）
3. 地图上可见所有活跃节点的位置、呼号、在线人数
4. 点击节点可查看接入地址（`udp://IP:60050`），输入 App 即可直接连接

### 9.8 nrlptt.com 官网服务器发现机制

服务器启动后自动向 `https://nrlptt.com` 官网上报自身信息，同时从官网下载全网服务器列表，作为 APRS 机制的补充/备用。

**上报接口（POST）：**

```text
POST https://nrlptt.com/api/platform-servers/report
Content-Type: application/json

{"name":"平台名称","host":"182.92.x.x","port":"60050","online":5,"total":20}
```

**获取列表接口（GET）：**

```text
GET https://nrlptt.com/api/platform-servers
```

返回全网所有已注册的 NRL 服务器列表，系统自动解析 `host:port`、过滤自身后加入互联转发列表。

| 字段 | 说明 |
| ---- | ---- |
| `name` | 平台名称（取 PlatformName 或 NameShorthand） |
| `host` | 服务器公网 IP 或域名 |
| `port` | UDP 语音端口（通常为 60050） |
| `online` | 当前在线设备数 |
| `total` | 历史最高在线设备数（峰值） |

---

## 十、协议与技术规格

| 项目 | 规格 |
| ---- | ---- |
| 传输协议 | UDP（语音/控制），TCP（Web 管理） |
| 语音编码 | G.711 (a-law) |
| 语音端口 | UDP 60050（公网直通） |
| 前端端口（对外） | TCP 80（前端容器，页面入口） |
| 后端接口端口 | TCP 9000（本机 / 局域网，不要暴露公网） |
| HTTPS | 可选，由部署者在宿主机自备反向代理指向前端容器 |
| 发言机制 | 抢占制（先到先得，语音结束后 1 秒释放） |
| 混音支持 | 会议模式（服务器端多路混音，不适用于 999 房间） |
| 数据存储 | SQLite |
| 容器支持 | Docker（前端 `ghcr.io/78ham/nrllink-web` + 后端 `ghcr.io/78ham/nrllink`） |
| 系统支持 | Linux、Windows、Docker |
| APRS 上报周期 | 立即 → 1 分钟 → 5 分钟 → 之后每 10 分钟 |
| APRS 服务发现周期 | 每 60 秒查询 aprs.tv，自动更新互联服务器列表 |
| APRS 节点类型标识 | NRLSRV（服务器）/ NRLBOX（硬件）/ NRLAPP（App）/ NRLMP（小程序） |
| 官网上报周期 | 启动立即，之后每 3 分钟（POST nrlptt.com） |
| 官网同步周期 | 启动立即，之后每 5 分钟（GET nrlptt.com） |
